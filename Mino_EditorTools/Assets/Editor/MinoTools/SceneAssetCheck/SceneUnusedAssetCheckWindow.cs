...
/// </summary>
public class SceneUnusedAssetCheckWindow : EditorWindow
{
    [Serializable]
    ...
    {
        SceneUnusedAssetCheckWindow window = GetWindow<SceneUnusedAssetCheckWindow>("场景未引用资源");
    ...
    private void MoveUnusedAssetsToNewFolder()
    {
        ...
        if (_moveFailures.Count > 0)
        {
            ...
            Debug.LogWarning("[SceneUnusedAssetCheckWindow] 资源转移失败详情：\n" + string.Join("\n", _moveFailures));
        }
    ...
    private void RestoreMovedAssetsToOriginalPaths()
    {
        ...
        if (_moveFailures.Count > 0)
        {
            ...
            Debug.LogWarning("[SceneUnusedAssetCheckWindow] 资源恢复失败详情：\n" + string.Join("\n", _moveFailures));
        }
